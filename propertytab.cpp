...
void PropertyTab::setParameter (settingsType aParameter, QString aSetting)
{
  switch (aParameter)
  {
...
void PropertyTab::setCurrentPropertyId ()
{
  ...
  Global::getInstance()->setCurrentPropertyId (index.data ().toInt());
}
...
void PropertyTab::on_editPropertySettings_clicked()
{
  ...
  dialog.setpropertyId(Global::getInstance()->getCurrentPropertyId());
  ...
  else
  {
    ...
    if (reply == QMessageBox::Yes)
    {
      ...
      query.bindValue(":id", QString::number(Global::getInstance()->getCurrentPropertyId()));
...
void PropertyTab::on_newAgenda_clicked()
{
  ...
  else
  {
    ...
    refreshOnSelected ();
    copyReportPattern ();
    checkAdvisers (Global::getInstance()->getCurrentPropertyId(), newSettings.getYear(), newSettings.getEtvNum());
  }
}
...
void PropertyTab::checkAdvisers (int aPropertyId, int aYear, int aEtvNum)
{
  if (1 == aEtvNum)
  {
    ...
    while (query.next())
    {
      ...
      if (reply == QMessageBox::Yes)
      {
        QSqlQuery insertQuery (QSqlDatabase::database(mUser));
        //set values
...
{
  Global::getInstance()->setCurrentEtvNumber(arg1.toInt());
}
...
void PropertyTab::on_changeAgendaSettings_clicked()
{
  ...
  newSettings.setYear (Global::getInstance()->getCurrentYear());
...
{
  Global::getInstance()->setCurrentYear(arg1.toInt());
  updateEtvBox ();
  updateInfoBox ();
}
